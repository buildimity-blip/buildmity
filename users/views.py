...
from .location_utils import calculate_distance, find_nearby_providers, calculate_eta, calculate_trip_price, get_address_from_coords, get_coords_from_address
# Forms
...
def signup_provider(request):
    if request.method == 'POST':
        ...
        print(f"Services in POST: {request.POST.getlist('services')}")
        ...
        if form.is_valid():
            ...
            user.service_radius = request.POST.get('service_radius', 10)
            user.save()
...
def update_request_status(request, request_id, status):
    ...
    if status == 'accepted':
        ...
        messages.success(request, 'You have accepted the job. Waiting for client payment.')
    ...
    # Update service need status if exists
...
def make_payment(request, request_id):
    ...
    if request.method == 'POST':
        ...
        amount_text = request.POST.get('amount', str(service_request.amount))
        ...
        payment, created = Payment.objects.get_or_create(
            ...
            defaults={
                ...
                'method': method if method != 'test' else 'simulate',
        ...
        if not created:
            payment.method = method if method != 'test' else 'simulate'
            payment.payer_phone_number = phone
            payment.amount = amount
            payment.status = Payment.PAID
            payment.paid_at = timezone.now()
            payment.transaction_id = f"TEST-{timezone.now().strftime('%Y%m%d%H%M%S')}"
            payment.save()
        ...
        AdminNotification.objects.create(
            ...
            message=f'Payment of UGX {amount} received from {request.user.username} for request #{service_request.id}',
...
def analytics_dashboard(request):
    ...
    for i in range(30):
        ...
        revenue_data.append({'date': date.strftime('%Y-%m-%d'), 'revenue': float(revenue)})
    ...
    for i in range(12):
        ...
        user_growth.append({'month': month.strftime('%B %Y'), 'users': new_users})
    ...
    context = {
        ...
        'user_growth': json.dumps(user_growth[::-1]),
...
def export_report(request, report_type):
    ...
    if report_type == 'users':
        ...
        for payment in Payment.objects.select_related('client', 'provider', 'service_request'):
            writer.writerow([
        ...
        for req in ServiceRequest.objects.select_related('client', 'provider', 'service'):
            writer.writerow([
...
def api_home(request):
    return Response({
        ...
        'endpoints': {
            ...
            'my_requests': '/api/my-requests/ (GET, POST)',
            'dashboard_stats': '/api/dashboard-stats/ (GET)',
            'search_providers': '/api/search-providers/ (GET)',
            'make_payment': '/api/make-payment/<id>/ (POST)',
...
from .location_utils import calculate_distance, find_nearby_providers, calculate_eta
...
def nearby_providers(request):
    """Get providers near user's location"""
    ...
    if not latitude or not longitude:
        ...
        if user.latitude and user.longitude:
            ...
            return Response({'error': 'Location not provided'}, status=400)
    
    nearby = find_nearby_providers(float(latitude), float(longitude), radius)
...
def calculate_trip(request):
    """Calculate distance and price between locations"""
    ...
    total_price = base_price + (distance * per_km_rate)
    ...
    return Response({
        ...
        'eta_minutes': eta,
        'estimated_price': round(total_price),
        'base_price': base_price,
        'per_km_rate': per_km_rate,
    })

from .location_utils import calculate_distance, find_nearby_providers, calculate_eta, calculate_trip_price

# ==================== LOCATION-BASED VIEWS ====================

@login_required
def update_location(request):
    """Update user's current location via POST"""
    if request.method == 'POST':
...
@login_required
def nearby_providers_map(request):
    """Show map with nearby providers"""
    services = Service.objects.filter(is_active=True)
    ...
    if user_lat and user_lng:
        nearby_providers = find_nearby_providers(
    ...
    return render(request, 'nearby_providers_map.html', context)


@login_required
def provider_location(request, provider_id):
    """View provider's location on map"""
    provider = get_object_or_404(User, id=provider_id, role='provider')
    
    # Calculate distance from client if client has location
    distance = None
    eta = None
    if request.user.latitude and request.user.longitude and provider.latitude and provider.longitude:
        distance = calculate_distance(
    ...
    context = {
        ...
        'user_lng': float(request.user.longitude) if request.user.longitude else None,
    }
    return render(request, 'provider_location.html', context)
...
def calculate_route(request):
    ...
    if request.method == 'POST':
        ...
        if all([pickup_lat, pickup_lng, dropoff_lat, dropoff_lng]):
            ...
            eta = calculate_eta(distance)
            price = calculate_trip_price(distance)
...
@login_required
def save_service_location(request, request_id):
    """Save pickup/dropoff locations for service request"""
    service_request = get_object_or_404(ServiceRequest, id=request_id, client=request.user)
    
    if request.method == 'POST':
        service_request.pickup_latitude = request.POST.get('pickup_lat')
        service_request.pickup_longitude = request.POST.get('pickup_lng')
        service_request.pickup_address = request.POST.get('pickup_address', '')
        service_request.dropoff_latitude = request.POST.get('dropoff_lat')
        service_request.dropoff_longitude = request.POST.get('dropoff_lng')
        service_request.dropoff_address = request.POST.get('dropoff_address', '')
        
        # Calculate distance
        if all([service_request.pickup_latitude, service_request.pickup_longitude,
                service_request.dropoff_latitude, service_request.dropoff_longitude]):
            service_request.distance_km = calculate_distance(
                float(service_request.pickup_latitude), float(service_request.pickup_longitude),
                float(service_request.dropoff_latitude), float(service_request.dropoff_longitude)
            )
            service_request.travel_time_minutes = calculate_eta(service_request.distance_km)
            
            # Auto-calculate price based on distance
            if service_request.amount == 0:
                service_request.amount = calculate_trip_price(service_request.distance_km)
        
        service_request.save()
        messages.success(request, 'Location saved successfully!')
        return redirect('service_request_detail', request_id=request_id)
    
    return render(request, 'save_location.html', {'service_request': service_request})
...
def dashboard(request):
    ...
    # CLIENT DASHBOARD
        ...
        payments = Payment.objects.filter(
            client=request.user
        ).select_related('service_request', 'provider').order_by('-created_at')[:10]
    ...
    print(f"Unknown role: {request.user.role}")
    context = {'user': request.user}
    return render(request, 'dashboard.html', context)

@api_view(['POST'])
@permission_classes([AllowAny])
def verify_flutterwave_payment(request, transaction_id):
    ...
    try:
        ...
        AdminNotification.objects.create(
            title='Payment Received',
            message=f'Payment of UGX {payment.amount} received from {payment.client.username}',
            notification_type='payment',
            related_user=payment.provider
        )
